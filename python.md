...
7. intersection() - Возвращает пересечение с другим множеством.
```python
s1.intersection(s2)
```
...
`Сериализация` — это процесс преобразования объекта в формат, который может быть легко сохранен или передан, например, в виде байтовой строки или текстового формата (как JSON). В Python для сериализации часто используются такие модули, как:
...
## zip
Функция zip() в Python используется для объединения нескольких итерируемых объектов (например, списков, кортежей) в последовательность из кортежей. Каждый кортеж содержит элементы, расположенные по одинаковым индексам во входных итерируемых объектах. Возвращает итератор, который можно преобразовать в список или другой тип.
```python
...
- Если итерируемые объекты разной длины, zip() остановится, когда закончится самый короткий из них.
- Можно распаковать результат zip() обратно в отдельные переменные:
```python
names, ages = zip(*list_of_tuples)
```
...
2. Итерация одновременно по двум спискам:
```python
...
`Класс в Python` — это шаблон для создания объектов, который объединяет данные (атрибуты) и функции (методы), которые работают с этими данными. Класс описывает состояние объекта - атрибуты данных и его поведение - методы для работы с атрибутами данных.
